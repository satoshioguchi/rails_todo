class TodosController < ApplicationController
  ...
  def index
    ...
    @finishs = Todo.where.not(status: 0)
    @category = Category.find_by(id: 1)
    @category_todo = @category.todos
    @category2 = Category.find_by(id: 2)
    @category_todo2 = @category2.todos
    @category3 = Category.find_by(id: 3)
    @category_todo3 = @category3.todos
  end
  ...
  private
    ...
    def todo_params
      params.require(:todo).permit(:title, :body, :priority, :category_id, :donetaskday, :group_id)
    end
end
